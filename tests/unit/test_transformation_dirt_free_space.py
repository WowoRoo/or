"""Tests for dirt transformation with free space above."""
...
def test_dirt_with_free_space_above_should_transform():
    """Test that dirt with free space above transforms to grass even without adjacent grass."""
    ...
    # Place dirt tile (no grass nearby)
    ...
    # Dirt should transform to grass (has free space above)
    assert count > 0
    assert dirt_tile.tile_type == TileType.GRASS


def test_dirt_surrounded_should_not_transform():
    """Test that dirt surrounded on all sides does NOT transform."""
    tile_list_ref = TileListReference(list(TileType))
    func_tile_list_ref = FunctionalTileListReference(list(FunctionalTileType))
