...
def zhang_suen(bitmap: WorkspaceBitmap) -> WorkspaceBitmap:
    ...
    # Invert: BACKGROUND = 1 should become foreground for skeletonization
    # We want to skeletonize the BACKGROUND (value 1)
    skeleton = bitmap.data.copy().astype(np.uint8)
    
    # Zhang-Suen works on 1 = foreground, 0 = background
    # Our bitmap has 1 = BACKGROUND, so we need to work with it as-is
...
def _zhang_suen_condition_1(skeleton: np.ndarray, x: int, y: int) -> bool:
    ...
    # Condition: p2 * p4 * p6 = 0 (at least one is background)
    return (p2 * p4 * p6) == 0
...
def _zhang_suen_condition_3(skeleton: np.ndarray, x: int, y: int) -> bool:
    ...
    # Condition: p2 * p4 * p8 = 0 (at least one is background)
    return (p2 * p4 * p8) == 0
